const news = require('./functions/getNews');
const twitter = require('./functions/getTweets');
...
api.get('/news', async(req, res) => {
    ...
    if(await (await prisma.latestnews.findMany({where: {type: {equals: "news"}}}))[0] === undefined || await (await prisma.latestnews.findMany({where: {type: {equals: "news"}}}))[0].lastmodified === 0 || await (await prisma.latestnews.findMany({where: {type: {equals: "news"}}}))[0].lastmodified === undefined){
        let n = news.getNews().catch((err) => {jsonres.success = false; jsonres.message = err; jsonres.status = "500"; res.send(jsonres)});
...
api.get('/twitter', async(req, res) => {
    ...
    // jsonres.content = (await (await n).get().catch((err) => {jsonres.success = false; jsonres.message = "Erreur lors de la récupération des tweets"; jsonres.status = "500"; jsonres.source = "error"; return console.log(err)}));
    jsonres.success = false;
    jsonres.message = "La fonctionnalité n'est pas encore disponible";
    jsonres.status = "500";
    jsonres.source = "error";
